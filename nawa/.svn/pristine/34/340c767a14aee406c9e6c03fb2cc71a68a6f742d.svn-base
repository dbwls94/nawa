...
var server = (function () {
	...
	var
		...
		// User Servlet
		setSignUp, setFollow, setUnFollow, getIsEmailExists, getRelationFollowing, getRelationFollowed, getUserProfile, getUserProfileData, setUserProfile, 
		// Search
			...
			if (asyncOption.async === false) {
				...
				if (asyncOption.callback) {
					...
					ajaxSettings.error = function (request, status, error) {
						if( jQuery.parseJSON(request.responseText).success === "1" ) 
							asyncOption.callback(jQuery.parseJSON(request.responseText));
						else 
	...
	//    성공시 {success:"1"}
	//    실패시 {success:"0", msg:[why failed]}
	setProject = function (pType, data, asyncOption) {
		var url = '/Project';
		
		var type = (pType=="create") ? 'POST' : 'PUT';
	...
	//    프로젝트 참가 여부 확인
	// example:
	//    server.getIsProjectParticipated({ projectId: '04055672-0248-41d4-9089-ab6078d37b20', 'email': 'test@test.com' }, { async: true, callback: function (response) { /* do... */ } });
	// retrun:
	//    성공시 {success:"1", isParticipated:true/false}
	//    실패시 {success:"0", msg:[why failed]}
	getIsProjectParticipated = function (data, asyncOption) {
		var url = '/Project/isParticipated/' + encodeURI(data.projectId) + "?email=" + data.email; // /Project/isParticipated/{projectId}
		var type = 'GET';
	...
	setUpdateComment = function (data, asyncOption) {
		var url = '/Comment?seq='+data.seq+'&projectId='+data.projectId+'&content=' + encodeURI(data.content); // /Comment
		var type = 'PUT';
	...
	setDeleteComment = function (data, asyncOption) {
		var url = '/Comment?seq=' + data.seq + "&projectId=" + data.projectId; // /Comment
		var type = 'DELETE';
	...
	setUnFollow = function (data, asyncOption) {
		var url = '/User/Relation/' + encodeURI(data.targetEmail); // /User/Relation
		var type = 'DELETE';
	...
	//
	// 목적:
	//    해당 사용자의 프로필 정보 - 관계, 프로젝트 등 전체 데이터
	// example:
	//    server.getUserProfile({ email: 'test@test.com' }, { async: true, callback: function (response) { /* do... */ } });
	// retrun:
	...
	//    실패시 { success: "0", msg: (why failed) }
	getUserProfileData = function (data, asyncOption) {
